...
**Phase Status**: 0/13 | **Est. Time**: 8-10 hours
...
**Expected Impact**: 50-70% reduction in re-renders, smoother audio playback
...
- [x] **1.2.1** Search project for all `console.log` occurrences
  - [x] Timer.jsx (lines ~167, 214, 225)
  - [x] util.js (line ~20, 24, 25)
  - [x] SecondaryButtons.jsx
  - [x] Any other components
...
- [x] **1.3.1** Remove commented code from Timer.jsx
  - [x] Lines ~75-77 (import statements)
  - [x] Lines ~124-135 (old cycleExpired logic)
  - [x] Lines ~180-191 (old commented button click handlers)
  - [x] Lines ~252-268 (old commented autostart logic)
- [x] **1.3.2** Remove incomplete `player()` utility function from util.js
  - [x] Verify it's not used anywhere
  - [x] Check if `react-use-audio-player` covers its functionality
...
**Expected Impact**: ~15-20% code reduction
...
```javascript
if (timerModeState === 1) setPomoTimeState()
if (timerModeState === 2) setShortTimeState()
if (timerModeState === 3) setLongTimeState()
```
...
- [x] **1.4.5** Extract logic into helper function:
  ```javascript
  const handleModeTransition = (newMode, shouldAutoStart)
  ```
...
**Expected Impact**: Reduced cyclomatic complexity, easier maintenance
...
- [x] **1.5.1** Convert function to use object/map instead of 6 if-statements
  ```javascript
  const buttonStyles = {
    `${isRunning}-${timerModeState}`: styleClass
  }
  ```
...
- [x] **1.6.1** Replace all `var` with `const` in Timer.jsx (lines ~34-39)
  - [x] `alarmVolume`
  - [x] `buttonSoundState`
  - [x] `btnStyle`
- [x] **1.6.2** Check util.js for any `var` usage
- [x] **1.6.3** Search entire codebase for remaining `var` statements
- [ ] **1.6.4** Ensure linting catches this (add ESLint rule)
...
- [ ] **1.7.1** Review naming conventions across components
- [ ] **1.7.2** Ensure consistent import/export style
- [ ] **1.7.3** Check for inconsistent spacing/indentation
- [ ] **1.7.4** Create `.prettierrc` config for consistency
- [ ] **1.7.5** Run Prettier on codebase
...
**Phase Status**: 0/14 | **Est. Time**: 6-8 hours

### 2.1 Audit settingsSlice.js

- [ ] **2.1.1** Review `settingsSlice.js` (need to see this file)
  - [ ] List all actions and reducers
  - [ ] Identify which ones actually modify state meaningfully
  - [ ] Check for redundant states (e.g., `setCurrentTime` with no params)
  - [ ] Look for actions that should be combined
- [ ] **2.1.2** Document current state structure
- [ ] **2.1.3** Create visual diagram of state flow
- [ ] **2.1.4** Identify problematic patterns:
  - [ ] Unused selectors
  - [ ] Actions that don't do what their name suggests
  - [ ] Multiple dispatches that should be one
- [ ] **2.1.5** Plan refactoring strategy

**Estimated Time**: 1-2 hours (discovery phase)

---

### 2.2 Create Composite Redux Actions

- [ ] **2.2.1** Identify common action sequences that happen together
  - [ ] Mode switching + time reset + expiry calculation
  - [ ] Auto-start cycle transitions
  - [ ] Pause/Resume with state preservation
- [ ] **2.2.2** Create composite thunk actions for these sequences
  - [ ] Example: `setTimerMode()` should auto-reset time
  - [ ] Example: `transitionMode()` should handle full cycle
- [ ] **2.2.3** Replace multiple sequential dispatches with single composite action
- [ ] **2.2.4** Update Timer.jsx to use new composite actions
...
- [ ] **2.3.1** Create new file: `src/hooks/useTimerMode.js`
- [ ] **2.3.2** Extract mode switching logic from Timer.jsx
  ```javascript
  const { currentMode, switchMode, getModeName } = useTimerMode()
  ```
- [ ] **2.3.3** Encapsulate all mode-related state updates
- [ ] **2.3.4** Handle transitions between modes
- [ ] **2.3.5** Test hook independently
- [ ] **2.3.6** Update Timer.jsx to use hook
...
- [ ] **2.4.1** Create new file: `src/hooks/useAutoStartCycle.js`
- [ ] **2.4.2** Extract auto-start cycle logic from Timer.jsx
- [ ] **2.4.3** Handle cycle transitions (pomo → short break → pomo → long break)
- [ ] **2.4.4** Manage counter increment/decrement
- [ ] **2.4.5** Encapsulate enable/disable logic
- [ ] **2.4.6** Test hook independently
- [ ] **2.4.7** Update Timer.jsx to use hook
...
- [ ] **2.5.1** Create new file: `src/hooks/useTimerControls.js`
- [ ] **2.5.2** Consolidate start/pause/resume button logic
- [ ] **2.5.3** Return clean interface:
  ```javascript
  const { 
    isRunning, 
    isPaused, 
  ...
  ```
- [ ] **2.5.4** Encapsulate button click sounds
- [ ] **2.5.5** Test all control scenarios
- [ ] **2.5.6** Update Timer.jsx to use hook
...
- [ ] **2.6.1** Remove custom hook logic (replaced by 2.3-2.5)
- [ ] **2.6.2** Update component to use new hooks
- [ ] **2.6.3** Reduce Timer.jsx from ~300 lines to ~150 lines
- [ ] **2.6.4** Focus component only on rendering and event handlers
- [ ] **2.6.5** Add JSDoc comments
- [ ] **2.6.6** Full component testing
...
- [ ] **2.7.1** Create `src/store/selectors.js` file
- [ ] **2.7.2** Define all selectors in one place:
  ```javascript
  export const selectTimerMode = state => state.settings.timermode
  export const selectIsRunning = state => state.settings.isrunning
  ```
- [ ] **2.7.3** Update all components to import selectors instead of inline
- [ ] **2.7.4** Add memoized selectors using Reselect library (if needed)
- [ ] **2.7.5** Document selector patterns
